...
public class FPSAiming : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		if (updateFrame)
		{
			...
			cam.LateUpdate();
			RotateCharacter();
			ik.solver.leftHandEffector.positionWeight = ((!(aimWeight > 0f) || !(sightWeight > 0f)) ? 0f : 1f);
	...
	private void LookDownTheSight()
	{
		...
		if (!(num <= 0f))
		{
			...
			ik.solver.Update();
			ik.solver.leftHandEffector.bone.rotation = gunTarget.rotation * quaternion;
			ik.solver.rightHandEffector.bone.rotation = gunTarget.rotation * quaternion2;
