...
public sealed class RuntimeAnalyzer : MonoBehaviour
{
	...
	private void OnAnalysisComplete()
	{
		...
		infoText.AppendFormat("Iterations: {0}, Max Failed Attempts: {1}", (!finishedEarly) ? analysis.TargetIterationCount : analysis.IterationCount, MaxFailedAttempts);
	...
	private void OnGUI()
	{
		...
		{
			string text = ((analysis.SuccessCount >= analysis.IterationCount) ? string.Empty : ("\nFailed Dungeons: " + (analysis.IterationCount - analysis.SuccessCount)));
