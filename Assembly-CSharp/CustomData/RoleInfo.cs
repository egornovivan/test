...
public class RoleInfo
{
	...
	public CustomCharactor.CustomData CreateCustomData()
	{
		...
		customData.sex = ((sex == 1) ? ESex.Female : ESex.Male);
		customData.appearData = new AppearData();
		customData.appearData.Deserialize(appearData);
		customData.nudeAvatarData = new AvatarData();
		customData.nudeAvatarData.Deserialize(nudeData);
		customData.charactorName = name;
		return customData;
	}
}
