...
public class Cos : FunctionNode
{
	...
	public override Var Calculate()
	{
		...
		if (A.value.type == EVarType.Vector || A.value.type == EVarType.Color || W.value.type == EVarType.Vector || W.value.type == EVarType.Color || X.value.type == EVarType.Vector || X.value.type == EVarType.Color || Y.value.type == EVarType.Vector || Y.value.type == EVarType.Color || B.value.type == EVarType.Vector || B.value.type == EVarType.Color)
		{
			...
			zero.x = vector.x * Mathf.Cos(vector2.x * vector5.x + vector3.x) + vector4.x;
			zero.y = vector.y * Mathf.Cos(vector2.y * vector5.y + vector3.y) + vector4.y;
			zero.z = vector.z * Mathf.Cos(vector2.z * vector5.z + vector3.z) + vector4.z;
			zero.w = vector.w * Mathf.Cos(vector2.w * vector5.w + vector3.w) + vector4.w;
			return zero;
		}
		float num = vector.x * Mathf.Cos(vector2.x * vector5.x + vector3.x) + vector4.x;
		return num;
	}
}
