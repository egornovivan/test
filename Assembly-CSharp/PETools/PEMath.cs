...
public static class PEMath
{
	...
	public static bool RayCastVoxel(Ray ray, out RaycastHit rch, int minvol = 1)
	{
		...
		float num2 = ((ray.direction.x > num) ? 1 : ((ray.direction.x < 0f - num) ? (-1) : 0));
		...
		float value = (int)((float)((int)ray.origin.x + 1) + num2 * 0.5f);
		float value2 = (int)((float)((int)ray.origin.y + 1) + num3 * 0.5f);
		float value3 = (int)((float)((int)ray.origin.z + 1) + num4 * 0.5f);
		...
		if (num2 != 0f)
		{
			for (float num5 = value; num5 > x - 0.1f && num5 <= x2 + 0.1f; num5 += num2)
			{
				float num6 = (num5 - ray.origin.x) / ray.direction.x;
				Vector3 vector = ray.origin + ray.direction * num6;
				if (voxels.Read(Mathf.FloorToInt(vector.x + num2 * 0.5f), Mathf.FloorToInt(vector.y), Mathf.FloorToInt(vector.z)).Volume >= minvol)
				{
					break;
				}
			}
