...
public class AstarPath : MonoBehaviour
{
	...
	private const int c_maxNodesInProcessGraph = 128;
	...
	public static bool SkipOptScanOnStartUp = false;
	...
	public static Version Version => new Version(3, 6);
	...
	private bool DrawUnwalkableNode(GraphNode node)
	{
		if (node is GridNode)
		{
			int clearance = ((GridNode)node).clearance;
			if (clearance >= AstarColor.WalkableNodes.Length)
			{
				int num = clearance - AstarColor.WalkableNodes.Length + 1;
				clearance = AstarColor.WalkableNodes.Length - 1;
				Color cyan = Color.cyan;
				cyan.r = 0.05f * (float)num;
				Gizmos.color = cyan;
			}
			else
			{
				Gizmos.color = AstarColor.WalkableNodes[clearance];
			}
			Gizmos.DrawCube((Vector3)node.position, Vector3.one * unwalkableNodeDebugSize);
		}
		else
		{
			Gizmos.color = (node.Walkable ? AstarColor.WalkableNodes[1] : AstarColor.WalkableNodes[0]);
			Gizmos.DrawCube((Vector3)node.position, Vector3.one * unwalkableNodeDebugSize);
		}
		return true;
	}
	...
	private void QueueGraphUpdatesInternal()
	{
		...
		while (graphUpdateQueue.Count > 0)
		{
			...
			foreach (IUpdatableGraph updateableGraph in astarData.GetUpdateableGraphs())
			{
				...
				if (navGraph.IntersectWithGraph(ref graphUpdateObject.bounds) && (graphUpdateObject.nnConstraint == null || graphUpdateObject.nnConstraint.SuitableGraph(active.astarData.GetGraphIndex(navGraph), navGraph)))
				{
	...
	private bool ProcessGraphUpdates(bool force)
	{
		...
		while (graphUpdateQueueRegular.Count > 0)
		{
			...
			try
			{
				num += item.graph.UpdateArea(item.obj);
			}
			catch (Exception ex2)
			{
	...
	private void ProcessGraphUpdatesAsync(object _astar)
	{
		...
		while (!astarPath.pathQueue.IsTerminating)
		{
			...
			while (graphUpdateQueueAsync.Count > 0)
			{
				...
				{
					Debug.LogWarning("Exception while updating graphs:\n" + ex);
				}
			}
			processingGraphUpdatesAsync.Set();
		}
	...
	public void Awake()
	{
		...
		if (!SkipOptScanOnStartUp && scanOnStartup && (!astarData.cacheStartup || astarData.file_cachedStartup == null))
		{
			Scan();
		}
	}
	...
	public void FloodFill(int graphMask = -1)
	{
	...
	public void InitializeNode(GraphNode node)
	{
		if (!pathQueue.AllReceiversBlocked)
		{
			Debug.LogError("Trying to initialize a node when it is not safe to initialize any nodes. Must be done during a graph update");
			return;
		}
		if (threadInfos == null)
		{
	...
	public void Scan(int graphMask = -1)
	{
		ScanLoop(null, graphMask);
	}

	public void ScanLoop(OnScanStatus statusCallback, int graphMask = -1)
	{
		if (graphs == null)
		{
			return;
