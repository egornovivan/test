...
public struct Matrix3X3
{
	...
	{
		return new IntVector3((int)(m.M11 * (float)v.x + m.M12 * (float)v.y + m.M13 * (float)v.z), (int)(m.M21 * (float)v.x + m.M22 * (float)v.y + m.M23 * (float)v.z), (int)(m.M31 * (float)v.x + m.M32 * (float)v.y + m.M33 * (float)v.z));
	}
}
