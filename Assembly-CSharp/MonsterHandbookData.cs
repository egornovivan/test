...
{
	private const int m_MhVersion = 1607200;
	...
	public static void LoadData()
	{
		...
		while (sqliteDataReader.Read())
		{
			...
			int strId = Convert.ToInt32(sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("Name")));
			int strId2 = Convert.ToInt32(sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("Content")));
			...
			monsterHandbookData = new MonsterHandbookData(num, array2, modelID, PELocalization.GetString(strId), PELocalization.GetString(strId2));
	...
	public static bool Deserialize(byte[] data)
	{
		...
		try
		{
			...
			using (BinaryReader binaryReader = new BinaryReader(input))
			{
				...
				if (num != 1607200 && PeGameMgr.IsSingle)
				{
					Debug.LogWarning($"MonsterHandbookData version valid! CurVersion:{1607200} ErrorVersion:{num}");
					return false;
				}
