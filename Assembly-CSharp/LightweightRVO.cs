...
public class LightweightRVO : MonoBehaviour
{
	...
	public void Update()
	{
		...
		for (int k = 0; k < agents.Count; k++)
		{
			...
			reference2 = Vector3.Lerp(interpolatedVelocities[k], agent.Velocity, agent.Velocity.magnitude * Time.deltaTime * 4f);
			...
			reference3 = vector4 + vector2 - vector3;
			ref Vector3 reference4 = ref verts[num + 1];
			reference4 = vector4 + vector2 + vector3;
			ref Vector3 reference5 = ref verts[num + 2];
			reference5 = vector4 - vector2 + vector3;
			ref Vector3 reference6 = ref verts[num + 3];
			reference6 = vector4 - vector2 - vector3;
