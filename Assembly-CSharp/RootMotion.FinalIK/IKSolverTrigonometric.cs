...
public class IKSolverTrigonometric : IKSolver
{
	...
	public override bool IsValid(bool log)
	{
		if (bone1.transform == null || bone2.transform == null || bone3.transform == null)
		{
			if (log)
			{
				LogWarning("Bone transform is null in IKSolverTrigonometric. Can't initiate solver.");
			}
			return false;
		}
		if (bone1.transform.position == bone2.transform.position)
		{
			if (log)
			{
				LogWarning("first bone position is the same as second bone position. Can't initiate solver.");
			}
			return false;
		}
		if (bone2.transform.position == bone3.transform.position)
		{
			if (log)
			{
				LogWarning("second bone position is the same as third bone position. Can't initiate solver.");
			}
			return false;
		}
