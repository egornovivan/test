...
public class IKSolverLookAt : IKSolver
{
	...
	public override bool IsValid(bool log)
	{
		if (!spineIsValid)
		{
			if (log)
			{
				LogWarning("IKSolverLookAt spine setup is invalid. Can't initiate solver.");
			}
			return false;
		}
		if (!headIsValid)
		{
			if (log)
			{
				LogWarning("IKSolverLookAt head transform is null. Can't initiate solver.");
			}
			return false;
		}
		if (!eyesIsValid)
		{
			if (log)
			{
				LogWarning("IKSolverLookAt eyes setup is invalid. Can't initiate solver.");
			}
			return false;
		}
		if (spineIsEmpty && headIsEmpty && eyesIsEmpty)
		{
			if (log)
			{
				LogWarning("IKSolverLookAt spine, head and eyes are empty. There is nothing for the solver to solve.");
			}
			return false;
		}
		...
		if (transform != null)
		{
			if (log)
			{
				LogWarning(transform.name + " is represented multiple times in a single IK chain. Can't initiate solver.");
			}
			return false;
		}
		...
		if (transform2 != null)
		{
			if (log)
			{
				LogWarning(transform2.name + " is represented multiple times in a single IK chain. Can't initiate solver.");
			}
			return false;
		}
		return true;
