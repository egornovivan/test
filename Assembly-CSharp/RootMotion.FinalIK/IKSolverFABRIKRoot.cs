...
public class IKSolverFABRIKRoot : IKSolver
{
	...
	public override bool IsValid(bool log)
	{
		...
		foreach (FABRIKChain fABRIKChain in array)
		{
			if (log)
			{
				if (!fABRIKChain.IsValid(base.LogWarning))
				{
					return false;
				}
			}
		...
		for (int j = 0; j < chains.Length; j++)
		{
			for (int k = 0; k < chains.Length; k++)
			{
				if (j != k && chains[j].ik == chains[k].ik)
				{
					if (log)
					{
						LogWarning(chains[j].ik.name + " is represented more than once in IKSolverFABRIKRoot chain.");
					}
					return false;
				}
			}
		...
		for (int l = 0; l < chains.Length; l++)
		{
			for (int m = 0; m < chains[l].children.Length; m++)
			{
				...
				if (num < 0)
				{
					if (log)
					{
						LogWarning(chains[l].ik.name + "IKSolverFABRIKRoot chain at index " + l + " has invalid children array. Child index is < 0.");
					}
					return false;
				}
				if (num == l)
				{
					if (log)
					{
						LogWarning(chains[l].ik.name + "IKSolverFABRIKRoot chain at index " + l + " has invalid children array. Child index is referencing to itself.");
					}
					return false;
				}
				if (num >= chains.Length)
				{
					if (log)
					{
						LogWarning(chains[l].ik.name + "IKSolverFABRIKRoot chain at index " + l + " has invalid children array. Child index > number of chains");
					}
					return false;
				}
				for (int n = 0; n < chains.Length; n++)
				{
					...
					for (int num2 = 0; num2 < chains[n].children.Length; num2++)
					{
						if (chains[n].children[num2] == l)
						{
							if (log)
							{
								LogWarning("Circular parenting. " + chains[n].ik.name + " already has " + chains[l].ik.name + " listed as it's child.");
							}
							return false;
						}
					}
				...
				for (int num3 = 0; num3 < chains[l].children.Length; num3++)
				{
					if (m != num3 && chains[l].children[num3] == num)
					{
						if (log)
						{
							LogWarning("Chain number " + num + " is represented more than once in the children of " + chains[l].ik.name);
						}
						return false;
					}
				}
