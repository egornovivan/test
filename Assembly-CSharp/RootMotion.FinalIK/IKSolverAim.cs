...
public class IKSolverAim : IKSolverHeuristic
{
	...
	private void RotateToTarget(Vector3 targetPosition, Bone bone, float weight)
	{
		if (weight >= 0f)
		{
			Quaternion b = Quaternion.FromToRotation(transformAxis, targetPosition - transform.position);
			bone.transform.rotation = Quaternion.Lerp(Quaternion.identity, b, weight) * bone.transform.rotation;
		}
		if (poleWeight > 0f)
		{
			Vector3 vector = polePosition - transform.position;
			Vector3 tangent = vector;
			Vector3 normal = transformAxis;
			Vector3.OrthoNormalize(ref normal, ref tangent);
			Quaternion b2 = Quaternion.FromToRotation(transformPoleAxis, tangent);
			bone.transform.rotation = Quaternion.Lerp(Quaternion.identity, b2, weight * poleWeight) * bone.transform.rotation;
		}
