...
public class AttPlusNPCData
{
	...
	public class Item
	{
		[DbReader.DbField("NPC", false)]
		public int _id;

		public AttrPlus AttPlus;

		[DbReader.DbField("PlusCount", false)]
		public int PlusCount;

		[DbReader.DbField("AttPlus", false)]
		private string attPlus
		{
			set
			{
	...
	private static AttribType GetRandMaxAttr(int npcId, PESkEntity entity, AttribType[] ChangeAbleAttr)
	{
		...
		for (int i = 0; i < ChangeAbleAttr.Length; i++)
		{
			...
			{
				float num2 = (entity.GetAttribute(ChangeAbleAttr[i], bSum: false) - (float)randomInt.m_Min) / (float)(randomInt.m_Max - randomInt.m_Min);
				if (num2 > num)
				{
