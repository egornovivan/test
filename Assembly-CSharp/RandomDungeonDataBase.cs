...
public class RandomDungeonDataBase
{
	...
	public static void LoadData()
	{
		...
		while (sqliteDataReader.Read())
		{
			...
			dungeonBaseData.monsterBuff = Convert.ToInt32(sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("monsterbuff")));
			...
			dungeonBaseData.bossMonsterBuff = Convert.ToInt32(sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("bossbuff")));
			...
			dungeonBaseData.minBossMonsterBuff = Convert.ToInt32(sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("minbossbuff")));
			...
			dungeonBaseData.rareItemTags = RandomDunGenUtil.GetIdWeightList(sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("rareItemtags")));
	...
	public static DungeonBaseData GetDataFromLevel(int level)
	{
		...
		int key = list[new System.Random().Next(list.Count)];
	...
	public static string GetRandomIsoTag(System.Random rand, List<IdWeight> weightPool)
	{
		...
		int num = RandomDunGenUtil.PickIdFromWeightList(rand, weightPool, 1)[0];
		if (num < DungeonBaseData.isoTagList.Count)
		{
			return DungeonBaseData.isoTagList[num];
		}
		return DungeonBaseData.isoTagList[DungeonBaseData.isoTagList.Count - 1];
	}
}
