...
public class PointGraph : NavGraph, IUpdatableGraph
{
	...
	private void ScanForMultiBlock()
	{
		...
		foreach (Transform item2 in root)
		{
			...
			{
				nodes[num2].SetPosition((Int3)item3.position);
				nodes[num2].Walkable = true;
				nodes[num2].gameObject = item3.gameObject;
				num2++;
			}
		}
		List<PointNode> list2 = new List<PointNode>(3);
		List<uint> list3 = new List<uint>(3);
		...
		for (int j = 0; j < count - 1; j++)
		{
			...
			for (int k = num3; k < num4; k++)
			{
				list2.Clear();
				list3.Clear();
				PointNode pointNode = nodes[k];
				for (int l = num3; l < num4; l++)
				{
					if (k != l)
					{
						PointNode pointNode2 = nodes[l];
						float dist = 0f;
						if (IsValidConnection(pointNode, pointNode2, out dist))
						{
							list2.Add(pointNode2);
							list3.Add((uint)Mathf.RoundToInt(dist * 1000f));
						}
					}
				}
				pointNode.connections = list2.ToArray();
				pointNode.connectionCosts = list3.ToArray();
			}
		}
	}
	...
	public int UpdateArea(GraphUpdateObject guo)
	{
		if (nodes == null)
		{
			return 0;
		...
		if (guo.updatePhysics)
		{
			...
			for (int j = 0; j < nodeCount; j++)
			{
				...
				for (int k = 0; k < nodeCount; k++)
				{
					...
					if (!flag && IsValidConnection(pointNode, pointNode2, out var dist))
					{
						if (list3 == null)
						{
							list.Clear();
