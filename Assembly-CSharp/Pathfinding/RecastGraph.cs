...
public class RecastGraph : NavGraph, IUpdatableGraph, IRaycastableGraph, INavmesh, INavmeshHolder, IFunnelGraph
{
	...
	public Vector3[] vectorVertices
	{
		get
		{
			...
			{
				ref Vector3 reference = ref _vectorVertices[i];
				reference = (Vector3)vertices[i];
			}
			return _vectorVertices;
		}
	}
	...
	public void SnapForceBoundsToScene()
	{
		...
		GetSceneMeshes(forcedBounds, tagMask, mask, list);
		if (list.Count != 0)
		{
	...
	public int UpdateArea(GraphUpdateObject guo)
	{
		...
		for (int num4 = a.xmin; num4 <= a.xmax; num4++)
		{
			for (int num5 = a.ymin; num5 <= a.ymax; num5++)
			{
				...
				for (int num6 = 0; num6 < nodes.Length; num6++)
				{
					nodes[num6].GraphIndex = num3;
				}
			}
		}
		a = a.Expand(1);
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			for (int j = 0; j < num2; j++)
			{
				...
				for (int k = 0; k < num; k++)
				{
					...
					reference = new Vector3((float)num6 * heightmapScale.z, heights[num5, num6] * y, (float)num5 * heightmapScale.x) + position;
	...
	private void CollectColliderMeshes(Bounds bounds, List<ExtraMesh> extraMeshes)
	{
		...
		{
			if (((1 << collider.gameObject.layer) & (int)mask) != 0 && collider.enabled && !collider.isTrigger && collider.bounds.Intersects(bounds))
			{
		...
		for (int i = 0; i < tileZCount; i++)
		{
			for (int j = 0; j < tileXCount; j++)
			{
				...
				for (int l = 0; l < navmeshTile.verts.Length; l++)
				{
					...
					reference = new Int3(reader.ReadInt32(), reader.ReadInt32(), reader.ReadInt32());
				}
