...
public class AIPath : MonoBehaviour
{
	...
	{
		if (Time.time - lastRepath >= repathRate && canSearchAgain && canSearch && (target != null || targetPosition != Vector3.zero))
		{
			SearchPath();
			return repathRate;
		}
	...
	public virtual void SearchPath()
	{
		...
		Vector3 end = ((!(target != null)) ? targetPosition : target.position);
		canSearchAgain = false;
		seeker.StartPath(GetFeetPosition(), end);
	}
	...
	public Vector3 CalculateVelocity(Vector3 currentPosition)
	{
		...
		Debug.DrawLine(vectorPath[currentWaypointIndex - 1], vectorPath[currentWaypointIndex], Color.black);
		Debug.DrawLine(GetFeetPosition(), vector2, Color.red);
		Debug.DrawRay(vector2, Vector3.up, Color.red);
		Debug.DrawRay(GetFeetPosition(), vector, Color.yellow);
		Debug.DrawRay(GetFeetPosition(), forward * num3, Color.cyan);
