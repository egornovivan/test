	...
	private class ProceduralTile
	{
		...
		private IEnumerator InternalGenerate()
		{
			...
			for (int i = 0; i < world.prefabs.Length; i++)
			{
				...
				if (pref.singleFixed)
				{
					GameObject ob = UnityEngine.Object.Instantiate(position: new Vector3(((float)x + 0.5f) * world.tileSize, 0f, ((float)z + 0.5f) * world.tileSize), original: pref.prefab, rotation: Quaternion.identity) as GameObject;
				...
				for (int j = 0; j < world.subTiles; j++)
				{
					...
					{
						float px = (float)x + (float)j / (float)world.subTiles;
						float pz = (float)z + (float)k / (float)world.subTiles;
						...
						float fcount = subSize * subSize * density + ditherMap[j + 1, k + 1];
						int count = Mathf.RoundToInt(fcount);
						ditherMap[j + 1 + 1, k + 1] += 0.4375f * (fcount - (float)count);
						ditherMap[j + 1 - 1, k + 1 + 1] += 0.1875f * (fcount - (float)count);
						ditherMap[j + 1, k + 1 + 1] += 0.3125f * (fcount - (float)count);
						ditherMap[j + 1 + 1, k + 1 + 1] += 0.0625f * (fcount - (float)count);
						for (int l = 0; l < count; l++)
						{
							GameObject ob2 = UnityEngine.Object.Instantiate(position: RandomInside(px, pz), original: pref.prefab, rotation: RandomYRot()) as GameObject;
