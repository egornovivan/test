...
public class VArtifactData
{
	...
	public void DestroyMaterials()
	{
		...
		if (!m_Options.ForEditor)
		{
			VAMaterial[] materials = m_Materials;
	...
	public int QueryVoxelType(VAMaterial vcmat)
	{
		...
		{
			if (voxel.Value.Type != 127)
			{
				int num = voxel.Value.Type & 0x7F;
				array[num]++;
			}
		}
		for (int l = 0; l < 64; l++)
	...
	public void SetVoxel(int pos, VCVoxel voxel)
	{
		...
		else if (m_Voxels.ContainsKey(pos))
		{
			m_Voxels[pos] = voxel;
		}
		else
	...
	public bool Import(byte[] buffer, VAOption options)
	{
		...
		if (version == 33685504 || version == 33685505 || version == 33554432 || version == 33619968)
		{
			...
			for (int l = 0; l < num4; l++)
			{
				...
				if (m_Options.ForEditor)
				{
					...
					VCEAssetMgr.s_TempDecals.Add(num9, vCDecalAsset);
