...
public static class VoxelGrassMeshComputer
{
	...
	private static int RandomCount(float expectation, VoxelGrassInstance vgi)
	{
		...
		return (int)((num2 == 0f) ? num : ((!(vgi.RandAttr.x < num2)) ? num : (num + 1f)));
	}
	...
	public static void ComputeMesh(List<VoxelGrassInstance> grass_list, List<VoxelGrassInstance> tri_grass_list, float density)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			int num7 = RandomCount(s_FullDensity * vgi.Density * density, vgi);
			for (int j = 0; j < num7; j++)
			{
				...
				reference = (reference2 = (reference3 = (reference4 = vgi.RandPos(j))));
				Vector3 normal = vgi.Normal;
				...
				reference9 = (reference10 = (reference11 = (reference12 = new Vector2((float)vgi.Prototype / 64f, 0f))));
				...
				reference13 = (reference14 = (reference15 = (reference16 = vgi.ColorDw)));
				num3++;
			}
		}
		...
		{
			VoxelGrassInstance vgi2 = tri_grass_list[k];
			int num12 = RandomCount(s_FullDensity * vgi2.Density * density * 0.333f, vgi2);
			for (int l = 0; l < num12; l++)
			{
				float num13 = vgi2.RandAttr.x * num4;
				Vector3 vector = vgi2.RandPos(l);
				for (int m = 0; m < 3; m++)
				{
					int num14 = num3 * 4;
					int num15 = num14 + 1;
					...
					reference24 = (reference25 = (reference26 = (reference27 = new Vector2((float)(vgi2.Prototype - 64) / 64f, (float)m * num5 + num13 + (vgi2.RandAttrs(m + 1).x - 0.5f) * num6))));
					...
					reference28 = (reference29 = (reference30 = (reference31 = vgi2.ColorDw)));
					num3++;
				}
			}
		}
	...
	public static void ComputeParticleMesh(VoxelGrassInstance[] grass_array, int count)
	{
		...
		{
			VoxelGrassInstance voxelGrassInstance = grass_array[i];
			int num = i * 4;
			int num2 = num + 1;
			int num3 = num + 2;
			int num4 = num + 3;
			...
			reference = (reference2 = (reference3 = (reference4 = voxelGrassInstance.RandPos(0))));
			Vector3 normal = voxelGrassInstance.Normal;
			...
			reference9 = (reference10 = (reference11 = (reference12 = new Vector2((float)voxelGrassInstance.Prototype / 64f, 0f))));
