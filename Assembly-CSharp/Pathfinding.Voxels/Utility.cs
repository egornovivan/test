...
public class Utility
{
	...
	public static int ClipPolygon(Vector3[] vIn, int n, Vector3[] vOut, float multi, float offset, int axis)
	{
		...
		for (; j < n; j++)
		{
			...
			{
				float num3 = array[num2] / (array[num2] - array[j]);
				ref Vector3 reference = ref vOut[num];
				reference = vIn[num2] + (vIn[j] - vIn[num2]) * num3;
				num++;
			}
			if (flag2)
			{
				ref Vector3 reference2 = ref vOut[num];
				reference2 = vIn[j];
				num++;
			}
			num2 = j;
	...
	public static int ClipPolygon(Int3[] vIn, int n, Int3[] vOut, int multi, int offset, int axis)
	{
		...
		for (; j < n; j++)
		{
			...
			if (flag != flag2)
			{
				...
				ref Int3 reference = ref vOut[num];
				reference = vIn[num2] + (vIn[j] - vIn[num2]) * num3;
				num++;
			}
			if (flag2)
			{
				ref Int3 reference2 = ref vOut[num];
				reference2 = vIn[j];
				num++;
			}
			num2 = j;
