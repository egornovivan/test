...
public class Util
{
	...
	{
		return CyclicDiff(high, low, period, skipWrap: false);
	}
	...
	{
		return CyclicDiff(high, low, 1f, skipWrap: false);
	}

	public static int CyclicDiff(int high, int low)
	{
		return CyclicDiff(high, low, 1, skipWrap: false);
	}
	...
	public static bool CyclicIsLower(float compared, float comparedTo, float reference, float period)
	{
		...
		if (CyclicDiff(compared, reference, period, skipWrap: true) < CyclicDiff(comparedTo, reference, period, skipWrap: true))
		{
			return true;
		}
		return false;
