...
public class System : HandleBase
{
	...
	public static RESULT create(out System studiosystem)
	{
		...
		studiosystem = null;
		rESULT = FMOD_Studio_System_Create(out var studiosystem2, 67076u);
		if (rESULT != 0)
		{
			return rESULT;
		}
	...
	public RESULT loadCommandReplay(string path, COMMANDREPLAY_FLAGS flags, out CommandReplay replay)
	{
		...
		RESULT rESULT = FMOD_Studio_System_LoadCommandReplay(rawPtr, Encoding.UTF8.GetBytes(path + '\0'), flags, out commandReplay);
		if (rESULT == RESULT.OK)
		{
