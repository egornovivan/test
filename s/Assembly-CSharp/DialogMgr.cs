...
public class DialogMgr
{
	...
	public bool SetQuest(int world_index, int npo_id, int quest_id, string text)
	{
		int key = WorldIndexAndNpoIdToKey(world_index, npo_id);
		if (!m_Quests.ContainsKey(key))
		{
	...
	public bool RemoveQuest(int world_index, int npo_id, int quest_id)
	{
		int key = WorldIndexAndNpoIdToKey(world_index, npo_id);
		if (m_Quests.ContainsKey(key))
		{
			return m_Quests[key].Remove(quest_id);
		}
		return false;
	}
