...
public class MissionRepository
{
	...
	public static bool HaveTalkED(int MissionID)
	{
		return HaveTalkED(MissionID, -1);
	}

	public static bool HaveTalkED(int MissionID, int TargetID)
	{
		MissionCommonData missionCommonData = MissionManager.GetMissionCommonData(MissionID);
		if (missionCommonData == null)
		{
	...
	{
		SqliteDataReader sqliteDataReader = LocalDatabase.Instance.ReadFullTable("Quest_List");
		...
		while (sqliteDataReader.Read())
		{
			...
			stringId = Convert.ToInt32(sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("VarDesc")));
			missionCommonData.m_MulDesc = PELocalization.GetString(stringId);
			...
			if (array.Length == 3)
			{
				missionCommonData.m_PujaReputationPre.type = Convert.ToInt32(array[0]);
				missionCommonData.m_PujaReputationPre.min = Convert.ToInt32(array[1]);
				missionCommonData.m_PujaReputationPre.max = Convert.ToInt32(array[2]);
			}
			@string = sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("ReputationPaja"));
			array = @string.Split('_');
			if (array.Length == 3)
			{
				missionCommonData.m_PajaReputationPre.type = Convert.ToInt32(array[0]);
				missionCommonData.m_PajaReputationPre.min = Convert.ToInt32(array[1]);
				missionCommonData.m_PajaReputationPre.max = Convert.ToInt32(array[2]);
			}
			...
			foreach (string text in array7)
			{
				...
				if (array8.Length == 2)
				{
					...
					{
						item9.npcs.Add(Convert.ToInt32(value));
					}
					item9.type = Convert.ToInt32(array8[1]);
					missionCommonData.m_npcType.Add(item9);
				}
			}
	...
	public static void LoadTypeMonster()
	{
		...
		while (sqliteDataReader.Read())
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				{
					typeMonsterData.m_CommonMonsterIds.Add(Convert.ToInt32(array2[0]));
					continue;
				}
				...
				if (array2.Length == 3)
				{
					type = Convert.ToInt32(array2[2]);
				}
				typeMonsterData.m_MonsterList.Add(item);
			}
			typeMonsterData.type = type;
			...
			for (int j = 0; j < array.Length; j++)
			{
				if (!(array[j] == "0"))
				{
					typeMonsterData.m_ReceiveList.Add(Convert.ToInt32(array[j]));
				}
			}
	...
	public static void LoadTypeFollow()
	{
		...
		while (sqliteDataReader.Read())
		{
			...
			int stringId = Convert.ToInt32(sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("Desc")));
			typeFollowData.m_Desc = PELocalization.GetString(stringId);
			...
			for (int i = 0; i < array.Length; i++)
			{
				if (!(array[i] == "0"))
				{
					stringId = Convert.ToInt32(array[i]);
					typeFollowData.m_iNpcList.Add(stringId);
				}
			}
	...
	public static void LoadTypeSearch()
	{
		...
		while (sqliteDataReader.Read())
		{
			...
			typeSearchData.m_NpcName = sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("LookName"));
	...
	public static void LoadTypeMessenger()
	{
		...
		while (sqliteDataReader.Read())
		{
			...
			typeMessengerData.m_iNpc = sqliteDataReader.GetInt32(sqliteDataReader.GetOrdinal("Npc"));
			typeMessengerData.m_iReplyNpc = sqliteDataReader.GetInt32(sqliteDataReader.GetOrdinal("ReplyNpc"));
	...
	public static void LoadTypeTowerDefends()
	{
		...
		while (sqliteDataReader.Read())
		{
			...
			int @int = sqliteDataReader.GetInt32(sqliteDataReader.GetOrdinal("Desc"));
			typeTowerDefendsData.m_Desc = PELocalization.GetString(@int);
