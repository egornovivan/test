...
public class StoreRepository
{
	...
	public static StoreData GetStoreData(int storeId)
	{
		if (dicShop.ContainsKey(storeId))
		{
			return dicShop[storeId];
		}
		return null;
	}

	public static StoreData GetNpcStoreData(int storeNpcid)
	{
		foreach (KeyValuePair<int, StoreData> item in dicShop)
		{
			StoreData value = item.Value;
			if (value.storenpcid == storeNpcid)
			{
				return value;
			}
		}
	...
	public static void LoadData()
	{
		...
		while (sqliteDataReader.Read())
		{
			...
			storeData.storenpcid = Convert.ToInt32(sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("storenpc_id")));
