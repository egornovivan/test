...
public static class RSubTerrUtils
{
	...
	{
		int x = Mathf.FloorToInt(world_pos.x / (float)RSubTerrConstant.ChunkSize);
		int z = Mathf.FloorToInt(world_pos.z / (float)RSubTerrConstant.ChunkSize);
		return ChunkPosToIndex(x, z);
	}

	public static int TreeChunkPosToIndex(Vector3 cnk_pos)
	{
		int num = Mathf.FloorToInt(cnk_pos.x + (float)RSubTerrConstant.ChunkSize);
		int num2 = Mathf.FloorToInt(cnk_pos.y);
		int num3 = Mathf.FloorToInt(cnk_pos.z + (float)RSubTerrConstant.ChunkSize);
	...
	public static int TreeWorldPosToChunkIndex(Vector3 world_pos, int cnk_index)
	{
		...
		int num = Mathf.FloorToInt(vector.x + (float)RSubTerrConstant.ChunkSize);
		int num2 = Mathf.FloorToInt(vector.y);
		int num3 = Mathf.FloorToInt(vector.z + (float)RSubTerrConstant.ChunkSize);
		return (num << 8) | (num2 << 16) | num3;
	}
