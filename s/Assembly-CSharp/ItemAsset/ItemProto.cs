...
public class ItemProto
{
	public class Bundle
	{
		...
		public List<MaterialItem> GetItems()
		{
			...
			for (int i = 0; i <= num; i++)
			{
				foreach (RandBundle mRand in mRandList)
				{
					if (UnityEngine.Random.value < mRand.probablity)
					{
						if (dictionary.ContainsKey(mRand.itemProtoId))
						{
							Dictionary<int, int> dictionary2;
							Dictionary<int, int> dictionary3 = (dictionary2 = dictionary);
							int itemProtoId;
							int key = (itemProtoId = mRand.itemProtoId);
							itemProtoId = dictionary2[itemProtoId];
							dictionary3[key] = itemProtoId + 1;
						}
						else
						{
							dictionary.Add(mRand.itemProtoId, 1);
						}
						break;
					}
				}
	...
	public class PropertyList : IEnumerable, IEnumerable<PropertyList.PropertyValue>
	{
		...
		public void AddProperty(AttribType property, float v)
		{
			...
			PropertyValue[] array = new PropertyValue[mPropertys.Length + 1];
			Array.Copy(mPropertys, array, mPropertys.Length);
			array[mPropertys.Length] = new PropertyValue
			{
				type = property,
				value = v
			};
	...
	public class Mgr
	{
		...
		{
			SqliteDataReader sqliteDataReader = LocalDatabase.Instance.ReadFullTable("PrototypeItem");
			...
			while (sqliteDataReader.Read())
			{
				...
				itemProto.englishName = sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("_engName"));
				itemProto.nameStringId = Convert.ToInt32(sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("_nameID")));
				...
				itemProto.icon = sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("_iconId"));
				...
				if (!string.IsNullOrEmpty(@string) && @string != "0")
				{
					...
					if (array.Length > 0)
					{
						...
						foreach (string text in array2)
						{
							...
							if (array3.Length == 2 && array3[0] == "EnergyMax")
							{
								itemProto.engergyMax = Convert.ToInt32(array3[1]);
							}
						}
					}
				}
				...
				itemProto.equipSex = (PeSex)Convert.ToInt32(sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("sex")));
				...
				string string2 = sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("category"));
				if (string2.ToLower() == "quest item")
				{
					itemProto.category = ItemCategory.IC_QuestItem;
				}
				else
				{
					itemProto.category = ItemCategory.IC_None;
				}
				string string3 = sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("learn"));
				if (string3 != "0")
				{
					string[] array4 = string3.Split(',');
					itemProto.replicatorFormulaIds = new int[array4.Length];
					for (int j = 0; j < array4.Length; j++)
					{
						itemProto.replicatorFormulaIds[j] = Convert.ToInt32(array4[j]);
					}
				}
				itemProto.propertyList = PropertyList.LoadFromDb(sqliteDataReader);
				string3 = sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("_repairMax"));
				itemProto.repairMaterialList = ConvertToMaterialItems(string3);
				string3 = sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("_strengthen"));
				itemProto.strengthenMaterialList = ConvertToMaterialItems(string3);
				string3 = sqliteDataReader.GetString(sqliteDataReader.GetOrdinal("bundle"));
				itemProto.bundle = Bundle.Load(string3);
		...
		public void ClearCreation()
		{
			...
			{
				if (mList[num].id >= CreationData.ObjectStartID)
				{
					mList.RemoveAt(num);
				}
			}
	...
	{
		ItemProto itemProto = Mgr.Instance.Get(id);
		return (itemProto == null) ? string.Empty : PELocalization.GetString(itemProto.nameStringId);
	}
	...
	public string GetName()
	{
		...
		{
			return PELocalization.GetString(nameStringId);
		}
		return englishName;
	}
	...
	{
		return Mgr.Instance.Get(id)?.icon;
	}
	...
	{
		return Mgr.Instance.GetByEditorType(editorType);
	}

	public static byte GetSetUp(int id)
	{
		return Mgr.Instance.Get(id)?.setUp ?? 0;
	}

	public static int GetPrice(int protoId)
	{
		return Mgr.Instance.Get(protoId).currencyValue;
	}

	public static List<MaterialItem> GetRepairMaterialList(int protoId)
	{
		if (Mgr.Instance.Get(protoId) == null)
		{
			return null;
		}
		return Mgr.Instance.Get(protoId).repairMaterialList;
	}
