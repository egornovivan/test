...
public class FarmPlantLogic
{
	...
	private const int Version = 20150409;
	...
	private double SecondsPerDay = 93600.0;

	public int BaseFactor => 4;
	...
	public void InitGrowRate(float extraRate)
	{
		float num = 0f;
		byte b = mTerrianType;
		mGrowRate = (float)BaseFactor * b switch
		{
			19 => 1f, 
			63 => 1.3f, 
			_ => 1f, 
		} + (float)BaseFactor * extraRate;
	...
	public void UpdateGrowRate(float extraRate, bool bReset = true)
	{
		...
		if (bReset)
		{
			mGrowRate = (float)BaseFactor * (num + extraRate);
			mExtraGrowRate = extraRate;
		}
		else
		{
			mGrowRate = (float)BaseFactor * (num + mExtraGrowRate + extraRate);
			mExtraGrowRate += extraRate;
		}
		UpdateStatus();
	}
	...
	public int GetHarvestItemNum()
	{
		UpdateStatus();
		return (int)((float)((int)(mLife / 20.0) + 1) * 0.2f * (float)mPlantInfo.mItemGetNum);
	}
		...
		for (int i = 0; i < itemGetNum; i++)
		{
			...
			for (int j = 0; j < mPlantInfo.mItemGetPro.Count; j++)
			{
				if (num < mPlantInfo.mItemGetPro[j].m_probablity)
				{
					...
					int id;
					int key = (id = mPlantInfo.mItemGetPro[j].m_id);
					id = dictionary2[id];
	...
	public void UpdateStatus()
	{
		...
		double timePassedDay = num / SecondsPerDay;
	...
	{
		mWater -= timePassedDay * (double)mPlantInfo.WaterDS;
	}

	public void UpdateClean(double timePassedDay)
	{
		mClean -= timePassedDay * (double)mPlantInfo.CleanDS;
	}

	public void UpdateGrowth(double timePassedSecond)
	{
		if (!mDead)
		{
	...
	public void UpdateLife(double timePassedDay)
	{
		...
		if (num >= 0.0 && num2 >= 0.0)
		{
			...
			mWater -= (double)mPlantInfo.WaterDS * num3;
			mWater = ((!(mWater < 0.0)) ? mWater : 0.0);
			mClean -= (double)mPlantInfo.CleanDS * num3;
		...
		if (timePassedDay < (double)Mathf.Epsilon)
		{
			return;
		}
		if (flag || (num < num2 && num > -2.0 && num2 > -2.0))
		{
			double num4 = mWater / (double)mPlantInfo.WaterDS;
			double num5 = (mClean - (double)mPlantInfo.mCleanLevel[0]) / (double)mPlantInfo.CleanDS;
			if (num4 >= num5)
			{
				double num3 = num5;
				if (num3 >= timePassedDay)
				...
				double num6 = (0.0 - 40.0 * (((double)(mPlantInfo.mWaterLevel[0] * 2f) - mWater * 2.0 + num3 * (double)mPlantInfo.WaterDS) / (double)mPlantInfo.mWaterLevel[0])) * num3 / 2.0;
				double num7 = 20.0 * num3;
				if (mLife + num6 + num7 <= (double)Mathf.Epsilon)
				{
					mLife = 0.0;
					mDead = true;
					return;
			...
			else
			{
				...
				double num9 = (0.0 - 40.0 * (((double)(mPlantInfo.mWaterLevel[0] * 2f) - mWater * 2.0 + num8 * (double)mPlantInfo.WaterDS) / (double)mPlantInfo.mWaterLevel[0])) * num8 / 2.0;
				double num10 = 20.0 * num8;
				if (mLife + num9 + num10 <= (double)Mathf.Epsilon)
				{
					mLife = 0.0;
					mDead = true;
					return;
				}
				mWater -= (double)mPlantInfo.WaterDS * num8;
				mWater = ((!(mWater < 0.0)) ? mWater : 0.0);
				mClean -= (double)mPlantInfo.CleanDS * num8;
				mClean = ((!(mClean < 0.0)) ? mClean : 0.0);
				mLife += num9 + num10;
				if (mLife > 100.0)
				{
				...
				mWater -= (double)mPlantInfo.WaterDS * num11;
				mWater = ((!(mWater < 0.0)) ? mWater : 0.0);
				mClean -= (double)mPlantInfo.CleanDS * num11;
				mClean = ((!(mClean < 0.0)) ? mClean : 0.0);
				mLife += num9 + num10;
				if (mLife > 100.0)
				{
		...
		{
			double num12 = mClean / (double)mPlantInfo.CleanDS;
			double num13 = (mWater - (double)mPlantInfo.mWaterLevel[0]) / (double)mPlantInfo.WaterDS;
			if (num12 >= num13)
			{
				double num3 = num13;
				if (num3 >= timePassedDay)
				...
				double num14 = (0.0 - 40.0 * (((double)(mPlantInfo.mCleanLevel[0] * 2f) - mClean * 2.0 + num3 * (double)mPlantInfo.CleanDS) / (double)mPlantInfo.mCleanLevel[0])) * num3 / 2.0;
				double num15 = 20.0 * num3;
				if (mLife + num15 + num14 <= (double)Mathf.Epsilon)
				{
					mLife = 0.0;
					mDead = true;
					return;
			...
			else
			{
				...
				double num17 = (0.0 - 40.0 * (((double)(mPlantInfo.mCleanLevel[0] * 2f) - mClean * 2.0 + num16 * (double)mPlantInfo.CleanDS) / (double)mPlantInfo.mCleanLevel[0])) * num16 / 2.0;
				double num18 = 20.0 * num16;
				if (mLife + num18 + num17 <= (double)Mathf.Epsilon)
				{
					mLife = 0.0;
					mDead = true;
					return;
				}
				mWater -= (double)mPlantInfo.WaterDS * num16;
				mWater = ((!(mWater < 0.0)) ? mWater : 0.0);
				mClean -= (double)mPlantInfo.CleanDS * num16;
				mClean = ((!(mClean < 0.0)) ? mClean : 0.0);
				mLife += num18 + num17;
				if (mLife > 100.0)
				{
				...
				mWater -= (double)mPlantInfo.WaterDS * num19;
				mWater = ((!(mWater < 0.0)) ? mWater : 0.0);
				mClean -= (double)mPlantInfo.CleanDS * num19;
				mClean = ((!(mClean < 0.0)) ? mClean : 0.0);
				mLife += num18 + num17;
				if (mLife > 100.0)
				{
		...
		else if (num2 < -2.0)
		{
			if (num < 0.0)
			{
				double num20 = mWater / (double)mPlantInfo.WaterDS;
				if (num20 > 0.0)
				{
					double num3 = num20;
					if (num3 >= timePassedDay)
					...
					double num21 = (0.0 - 40.0 * (((double)(mPlantInfo.mWaterLevel[0] * 2f) - mWater * 2.0 + num3 * (double)mPlantInfo.WaterDS) / (double)mPlantInfo.mWaterLevel[0])) * num3 / 2.0;
					double num22 = 20.0 * num3;
					if (mLife + num21 + num22 <= (double)Mathf.Epsilon)
					{
						mLife = 0.0;
						mDead = true;
						return;
		...
		if (timePassedDay < (double)Mathf.Epsilon || (!flag2 && (!(num < 0.0) || !(num2 < 0.0))))
		{
			return;
		}
		if (mWater > 0.0 && mClean > 0.0)
		{
			double num23 = mWater / (double)mPlantInfo.WaterDS;
			double num24 = mClean / (double)mPlantInfo.CleanDS;
			...
			double num25 = (0.0 - 40.0 * (((double)(mPlantInfo.mWaterLevel[0] * 2f) - mWater * 2.0 + num3 * (double)mPlantInfo.WaterDS) / (double)mPlantInfo.mWaterLevel[0])) * num3 / 2.0;
			double num26 = (0.0 - 40.0 * (((double)(mPlantInfo.mCleanLevel[0] * 2f) - mClean * 2.0 + num3 * (double)mPlantInfo.CleanDS) / (double)mPlantInfo.mCleanLevel[0])) * num3 / 2.0;
			if (mLife + num25 + num26 <= (double)Mathf.Epsilon)
			{
				mLife = 0.0;
				mDead = true;
				return;
		...
		if (mWater > 0.0 && mClean <= (double)Mathf.Epsilon)
		{
			double num3 = mWater / (double)mPlantInfo.WaterDS;
			if (num3 >= timePassedDay)
			{
				num3 = timePassedDay;
				timePassedDay = 0.0;
			...
			double num27 = (0.0 - 40.0 * (((double)(mPlantInfo.mWaterLevel[0] * 2f) - mWater * 2.0 + num3 * (double)mPlantInfo.WaterDS) / (double)mPlantInfo.mWaterLevel[0])) * num3 / 2.0;
			double num28 = -40.0 * num3;
			if (mLife + num27 + num28 <= (double)Mathf.Epsilon)
			{
				mLife = 0.0;
				mDead = true;
			}
		...
		else if (mWater <= (double)Mathf.Epsilon && mClean > 0.0)
		{
			double num3 = mClean / (double)mPlantInfo.CleanDS;
			if (num3 >= timePassedDay)
			{
				num3 = timePassedDay;
				timePassedDay = 0.0;
			...
			double num29 = (0.0 - 40.0 * (((double)(mPlantInfo.mCleanLevel[0] * 2f) - mClean * 2.0 + num3 * (double)mPlantInfo.CleanDS) / (double)mPlantInfo.mCleanLevel[0])) * num3 / 2.0;
			double num30 = -40.0 * num3;
			if (mLife + num30 + num29 <= (double)Mathf.Epsilon)
			{
				mLife = 0.0;
				mDead = true;
				return;
	...
	public double CountNextUpdateTime()
	{
		if (!mDead && !IsRipe)
		{
			...
			{
				double num5 = CountNextDeadTime() * SecondsPerDay;
				if (num5 < num)
				{
					num = num5;
				}
	...
	public double CountNextWatertime()
	{
		...
		return num / (double)mPlantInfo.WaterDS;
	}
	...
	public double CountNextCleanTime()
	{
		...
		return num / (double)mPlantInfo.CleanDS;
	}
	...
	public double CountNextDeadTime()
	{
		...
		if (num2 >= 0.0 && num3 >= 0.0)
		{
			...
			num += num7;
			num4 -= (double)mPlantInfo.WaterDS * num7;
			num4 = ((!(num4 < 0.0)) ? num4 : 0.0);
			num5 -= (double)mPlantInfo.CleanDS * num7;
		...
		{
			double num8 = num4 / (double)mPlantInfo.WaterDS;
			double num9 = (num5 - (double)mPlantInfo.mCleanLevel[0]) / (double)mPlantInfo.CleanDS;
			if (num8 >= num9)
			{
				double num7 = num9;
				double num10 = (0.0 - 40.0 * (((double)(mPlantInfo.mWaterLevel[0] * 2f) - num4 * 2.0 + num7 * (double)mPlantInfo.WaterDS) / (double)mPlantInfo.mWaterLevel[0])) * num7 / 2.0;
				double num11 = 20.0 * num7;
				if (num6 + num10 + num11 <= (double)Mathf.Epsilon)
				{
					double a = -40f * mPlantInfo.WaterDS / mPlantInfo.mWaterLevel[0] / 2f;
			...
			else
			{
				...
				double num14 = (0.0 - 40.0 * (((double)(mPlantInfo.mWaterLevel[0] * 2f) - num4 * 2.0 + num12 * (double)mPlantInfo.WaterDS) / (double)mPlantInfo.mWaterLevel[0])) * num12 / 2.0;
				double num15 = 20.0 * num12;
				if (num6 + num14 + num15 <= (double)Mathf.Epsilon)
				{
					double a2 = -40f * mPlantInfo.WaterDS / mPlantInfo.mWaterLevel[0] / 2f;
				...
				num4 -= (double)mPlantInfo.WaterDS * num12;
				num4 = ((!(num4 < 0.0)) ? num4 : 0.0);
				num5 -= (double)mPlantInfo.CleanDS * num12;
				num5 = ((!(num5 < 0.0)) ? num5 : 0.0);
				num6 += num14 + num15;
				num += num12;
				num14 = -40.0 * num13;
				...
				num4 -= (double)mPlantInfo.WaterDS * num13;
				num4 = ((!(num4 < 0.0)) ? num4 : 0.0);
				num5 -= (double)mPlantInfo.CleanDS * num13;
				num5 = ((!(num5 < 0.0)) ? num5 : 0.0);
				num6 += num14 + num15;
				num += num13;
			}
		...
		{
			double num17 = num5 / (double)mPlantInfo.CleanDS;
			double num18 = (num4 - (double)mPlantInfo.mWaterLevel[0]) / (double)mPlantInfo.WaterDS;
			if (num17 >= num18)
			{
				double num7 = num18;
				double num19 = (0.0 - 40.0 * (((double)(mPlantInfo.mCleanLevel[0] * 2f) - num5 * 2.0 + num7 * (double)mPlantInfo.CleanDS) / (double)mPlantInfo.mCleanLevel[0])) * num7 / 2.0;
				double num20 = 20.0 * num7;
				if (num6 + num20 + num19 <= (double)Mathf.Epsilon)
				{
					double a3 = -40f * mPlantInfo.CleanDS / mPlantInfo.mCleanLevel[0] / 2f;
			...
			else
			{
				...
				double num23 = (0.0 - 40.0 * (((double)(mPlantInfo.mCleanLevel[0] * 2f) - num5 * 2.0 + num21 * (double)mPlantInfo.CleanDS) / (double)mPlantInfo.mCleanLevel[0])) * num21 / 2.0;
				double num24 = 20.0 * num21;
				if (num6 + num24 + num23 <= (double)Mathf.Epsilon)
				{
					double a4 = -40f * mPlantInfo.CleanDS / mPlantInfo.mCleanLevel[0] / 2f;
				...
				num4 -= (double)mPlantInfo.WaterDS * num21;
				num4 = ((!(num4 < 0.0)) ? num4 : 0.0);
				num5 -= (double)mPlantInfo.CleanDS * num21;
				num5 = ((!(num5 < 0.0)) ? num5 : 0.0);
				num6 += num24 + num23;
				num += num21;
				num23 = -40.0 * num22;
				...
				num4 -= (double)mPlantInfo.WaterDS * num22;
				num4 = ((!(num4 < 0.0)) ? num4 : 0.0);
				num5 -= (double)mPlantInfo.CleanDS * num22;
				num5 = ((!(num5 < 0.0)) ? num5 : 0.0);
				num6 += num24 + num23;
				num += num22;
			}
		...
		else if (num3 < -2.0)
		{
			if (num2 < 0.0)
			{
				double num26 = num4 / (double)mPlantInfo.WaterDS;
				if (num26 > 0.0)
				{
					double num7 = num26;
					double num27 = (0.0 - 40.0 * (((double)(mPlantInfo.mWaterLevel[0] * 2f) - num4 * 2.0 + num7 * (double)mPlantInfo.WaterDS) / (double)mPlantInfo.mWaterLevel[0])) * num7 / 2.0;
					double num28 = 20.0 * num7;
					if (num6 + num27 + num28 <= (double)Mathf.Epsilon)
					{
						double a5 = -40f * mPlantInfo.WaterDS / mPlantInfo.mWaterLevel[0] / 2f;
		...
		if (flag2 || (num2 < 0.0 && num3 < 0.0))
		{
			...
			{
				double num30 = num4 / (double)mPlantInfo.WaterDS;
				double num31 = num5 / (double)mPlantInfo.CleanDS;
				double num7 = ((!(num30 > num31)) ? num30 : num31);
				double num32 = (0.0 - 40.0 * (((double)(mPlantInfo.mWaterLevel[0] * 2f) - num4 * 2.0 + num7 * (double)mPlantInfo.WaterDS) / (double)mPlantInfo.mWaterLevel[0])) * num7 / 2.0;
				double num33 = (0.0 - 40.0 * (((double)(mPlantInfo.mCleanLevel[0] * 2f) - num5 * 2.0 + num7 * (double)mPlantInfo.CleanDS) / (double)mPlantInfo.mCleanLevel[0])) * num7 / 2.0;
				if (num6 + num32 + num33 <= (double)Mathf.Epsilon)
				{
					double a6 = -40f * (mPlantInfo.WaterDS / mPlantInfo.mWaterLevel[0] + mPlantInfo.CleanDS / mPlantInfo.mCleanLevel[0]) / 2f;
			...
			if (num4 > 0.0 && num5 <= (double)Mathf.Epsilon)
			{
				double num7 = num4 / (double)mPlantInfo.WaterDS;
				double num34 = (0.0 - 40.0 * (((double)(mPlantInfo.mWaterLevel[0] * 2f) - num4 * 2.0 + num7 * (double)mPlantInfo.WaterDS) / (double)mPlantInfo.mWaterLevel[0])) * num7 / 2.0;
				double num35 = -40.0 * num7;
				if (num6 + num34 + num35 <= (double)Mathf.Epsilon)
				{
					double a7 = -40f * (mPlantInfo.WaterDS / mPlantInfo.mWaterLevel[0]) / 2f;
			...
			else if (num4 <= (double)Mathf.Epsilon && num5 > 0.0)
			{
				double num7 = num5 / (double)mPlantInfo.CleanDS;
				double num36 = (0.0 - 40.0 * (((double)(mPlantInfo.mCleanLevel[0] * 2f) - num5 * 2.0 + num7 * (double)mPlantInfo.CleanDS) / (double)mPlantInfo.mCleanLevel[0])) * num7 / 2.0;
				double num37 = -40.0 * num7;
				if (num6 + num37 + num36 <= (double)Mathf.Epsilon)
				{
					double a8 = -40f * (mPlantInfo.CleanDS / mPlantInfo.mCleanLevel[0]) / 2f;
	...
	{
		if (mLastUpdateTime < (double)Mathf.Epsilon && mLastUpdateTime > (double)(0f - Mathf.Epsilon))
		{
